...
import { db, auth } from "@/lib/firebase";                   // ← auth を使う
...
export default function Home() {
  ...
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, (user) => {
      if (!user) {
        setLoading(false); // スピナーを止める
        router.replace("/login?next=/orders"); // 戻るで戻れないよう replace
      }
    });
    return () => unsub();
  }, [router]);
    ...
    <div className="relative">
      ...
      <header className="fixed inset-x-0 top-14 z-40 bg-gradient-to-r from-teal-500 to-pink-500 shadow-md">
        <div className="mx-auto flex h-14 max-w-screen-lg items-center px-4">
          <h1 className="text-white text-lg md:text-xl font-semibold">注文一覧</h1>
          <div className="ml-auto flex items-center gap-2">
            <div className="flex md:hidden gap-1">
              <Link href="/estimateOrders">
                <Button variant="ghost" className="text-white hover:text-white" size="icon">
            ...
            <div className="hidden md:flex gap-2">
              ...
              <Button variant="ghost" className="text-white hover:text-white" onClick={openSumDialog}>
      ...
      <main className="mx-auto max-w-screen-lg px-4 pt-[140px] pb-10">
        <div className="mb-4 flex items-center justify-between rounded-md border bg-gray-50 px-3 py-3 md:hidden">
          <p className="text-sm">合計点数: <b>{totalQuan}</b></p>
          <p className="text-sm">合計金額: <b>¥{totalPrice.toLocaleString()}</b></p>
        </div>
          ...
          <table className="w-full overflow-hidden rounded-md border text-left">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-4 py-3 text-lg font-semibold text-right">合計点数: {totalQuan}</th>
                <th className="px-4 py-3 text-lg font-semibold text-right">合計金額: ¥{totalPrice.toLocaleString()}</th>
            ...
            <tbody>
              {orders.map((o) => (
                <tr key={o.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-base">{o.createdAt?.toDate().toLocaleString()}</td>
                  <td className="px-4 py-3 text-right">{o.totalItems}</td>
                  <td className="px-4 py-3 text-right">¥{o.totalPrice.toLocaleString()}</td>
                  <td className="px-4 py-3 text-right">
                    <Button size="sm" variant="default" onClick={() => handleDetail(o.id)}>詳細</Button>
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        <div className="flex flex-col gap-3 md:hidden">
          {orders.map((o) => (
            <div key={o.id} className="rounded-md border bg-white p-4 shadow-sm hover:shadow transition">
              <div className="mb-2 flex items-center justify-between">
                <p className="font-semibold">注文No. {o.orderNo}</p>
                <p className="text-sm text-gray-600">{o.createdAt?.toDate().toLocaleString()}</p>
              </div>
